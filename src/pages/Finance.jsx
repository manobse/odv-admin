  ...
  return (
    <div>
      <PageHeader title="Income" sub="All revenue entries"
        action={<Btn size="sm" onClick={() => { setForm({ date: today(), category: cats[0]?._id || '', amount:'', description:'' }); setModal(true) }}>＋ Add Income</Btn>}
      />
  ...
  return (
    <div>
      <PageHeader title="Expenses" sub="Daily expense tracking"
        action={<Btn size="sm" onClick={() => { setForm({ date: today(), category: cats[0]?._id || '', amount:'', description:'' }); setModal(true) }}>＋ Add Expense</Btn>}
      />
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(170px,1fr))', gap:14, marginBottom:18 }}>
